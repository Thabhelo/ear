...
import {
  LiveKitCallSession,
  MuteToggle,
  RemotePresenceBridge,
  useDisconnectRoom
} from "./LiveKitCallSession";
...
type Stage = "ready" | "connecting" | "waiting" | "connected" | "ended" | "disconnected";
...
export function CallFlow() {
  ...
  const handleConnected = useCallback(() => {
    // Joined the room. Stay calm until the other person is in too.
    setStage((current) => (current === "connecting" ? "waiting" : current));
  }, []);

  const handlePresence = useCallback((remoteCount: number) => {
    setStage((current) => {
      if (current === "waiting" && remoteCount > 0) return "connected";
      if (current === "connected" && remoteCount === 0) return "waiting";
      return current;
    });
  }, []);
  ...
  const handleDisconnected = useCallback((reason?: DisconnectReason) => {
    ...
    if (reason === DisconnectReason.CLIENT_INITIATED) {
      // We asked to leave; this is not a failure.
      setStage("ended");
      return;
    }
    setStage("disconnected");
    setError("The call dropped. You can join again when you're ready.");
  }, []);

  const handleRoomError = useCallback(() => {
    if (leavingRef.current) return;
    // Only treat errors as fatal while we are still trying to connect.
    if (stageRef.current === "connecting") {
      setCredentials(null);
      setStage("disconnected");
      setError("We couldn't connect your call. Check your connection and try again.");
    }
  }, []);
  ...
  const heroSub =
    stage === "ended"
      ...
      : stage === "disconnected"
        ...
        : stage === "waiting"
          ? "The other person will join in a moment."
  ...
  const bodyCopy =
    stage === "ready"
      ...
      : stage === "connecting"
        ...
        : stage === "waiting"
          ? "Take a breath and get comfortable. You'll hear them the moment they arrive."
          : stage === "connected"
    ...
    >
      <Surface style={{ padding: 40, textAlign: "center" }}>
        ...
        <div className="flex justify-center items-center" style={{ gap: 12, flexWrap: "wrap" }}>
          ...
          {(stage === "connected" || stage === "waiting") && credentials ? (
            <CallControls busy={busy} onLeave={leaveCall} />
          ) : null}

          {stage === "connected" && !credentials ? (
            <button
              type="button"
              onClick={leaveCall}
              style={{
  ...
  return (
    <PageShell active="/queue" maxWidth={640}>
      ...
      {credentials ? (
        ...
        >
          <RemotePresenceBridge onChange={handlePresence} />
          {callCard}
        </LiveKitCallSession>
      ) : (
        callCard
